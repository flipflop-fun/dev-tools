import { useState } from "react";
import { Keypair } from "@solana/web3.js";
import bs58 from "bs58";

export default function Base58SecretTool() {
  const [base58, setBase58] = useState("");
  const [arrayText, setArrayText] = useState<string>("");
  const [address, setAddress] = useState<string>("");
  const [error, setError] = useState<string>("");
  const [hideInput, setHideInput] = useState(true);
  const [hideArray, setHideArray] = useState(true);
  const [copiedBase58, setCopiedBase58] = useState(false);
  const [copiedArray, setCopiedArray] = useState(false);
  const [copiedAddress, setCopiedAddress] = useState(false);

  const onConvert = () => {
    setError("");
    setArrayText("");
    setAddress("");
    try {
      const bytes = bs58.decode(base58.trim());
      if (!(bytes.length === 64 || bytes.length === 32)) {
        throw new Error("Expected a base58 secret key of 32 or 64 bytes");
      }
      const kp = bytes.length === 64 ? Keypair.fromSecretKey(bytes) : Keypair.fromSeed(bytes);
      setAddress(kp.publicKey.toBase58());
      setArrayText("[" + Array.from(kp.secretKey).join(",") + "]");
    } catch (e: unknown) {
      setError(e instanceof Error ? e.message : String(e));
    }
  };

  return (
    <div className="space-y-4">
      <h2 className="text-lg font-semibold">Base58 → Array Private Key & Address</h2>
      <div className="flex items-center gap-2">
        <input
          type={hideInput ? "password" : "text"}
          className="w-full rounded-md border border-neutral-700 bg-neutral-900 px-3 py-2 text-sm outline-none focus:border-neutral-500"
          placeholder="Base58 secret key"
          value={base58}
          onChange={(e) => setBase58(e.target.value)}
        />
        <button
          aria-label={hideInput ? "Show" : "Hide"}
          className="p-2 rounded-md border border-neutral-700"
          onClick={() => setHideInput((v) => !v)}
        >
          {hideInput ? (
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="h-4 w-4">
              <path d="M12 5c-7 0-10 7-10 7s3 7 10 7 10-7 10-7-3-7-10-7zm0 12a5 5 0 110-10 5 5 0 010 10z"/>
              <circle cx="12" cy="12" r="3" />
            </svg>
          ) : (
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="h-4 w-4">
              <path d="M2 12s3 7 10 7c2.2 0 4.1-.6 5.7-1.5l2.8 2.8 1.4-1.4L4.4 3.4 3 4.8l3.1 3.1C4.4 9.2 3 11.1 2 12zm9.9-5c.1 0 .1 0 0 0l6.1 6.1c1.2-1.3 2-2.7 2-2.7s-3-7-10-7c-.6 0-1.2 0-1.7.1l1.7 1.6c.6-.1 1.2-.1 1.9-.1z"/>
            </svg>
          )}
        </button>
        <button
          aria-label="Copy"
          title="Copy"
          className="p-2 rounded-md border border-neutral-700"
          onClick={() => {
            void navigator.clipboard
              .writeText(base58)
              .then(() => {
                setCopiedBase58(true);
                setTimeout(() => setCopiedBase58(false), 1500);
              });
          }}
        >
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="h-4 w-4">
            <path d="M16 1H4c-1.1 0-2 .9-2 2v12h2V3h12V1zm3 4H8c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h11c1.1 0 2-.9 2-2V7c0-1.1-.9-2-2-2zm0 14H8V7h11v12z"/>
          </svg>
        </button>
        {copiedBase58 && <div className="text-xs text-green-400">Copied!</div>}
      </div>

      <div className="flex gap-2">
        <button
          className="px-3 py-2 rounded-md bg-indigo-600 hover:bg-indigo-500 text-white text-sm"
          onClick={onConvert}
        >
          Convert
        </button>
        <button
          className="px-3 py-2 rounded-md border border-neutral-700 text-sm"
          onClick={() => {
            setBase58("");
            setArrayText("");
            setAddress("");
            setError("");
          }}
        >
          Clear
        </button>
      </div>

      {error && <div className="text-red-400 text-sm">{error}</div>}

      {!!arrayText && (
        <div className="space-y-2">
          <div>
            <div className="text-xs text-neutral-400">Array Private Key</div>
            <div className="flex items-center gap-2">
              <div className="flex-1 break-all rounded-md border border-neutral-700 bg-neutral-900 px-3 py-2 text-sm">
                {hideArray ? "•".repeat(arrayText.length) : arrayText}
              </div>
              <button
                aria-label={hideArray ? "Show" : "Hide"}
                className="p-2 rounded-md border border-neutral-700"
                onClick={() => setHideArray((v) => !v)}
              >
                {hideArray ? (
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="h-4 w-4">
                    <path d="M12 5c-7 0-10 7-10 7s3 7 10 7 10-7 10-7-3-7-10-7zm0 12a5 5 0 110-10 5 5 0 010 10z"/>
                    <circle cx="12" cy="12" r="3" />
                  </svg>
                ) : (
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="h-4 w-4">
                    <path d="M2 12s3 7 10 7c2.2 0 4.1-.6 5.7-1.5l2.8 2.8 1.4-1.4L4.4 3.4 3 4.8l3.1 3.1C4.4 9.2 3 11.1 2 12zm9.9-5c.1 0 .1 0 0 0l6.1 6.1c1.2-1.3 2-2.7 2-2.7s-3-7-10-7c-.6 0-1.2 0-1.7.1l1.7 1.6c.6-.1 1.2-.1 1.9-.1z"/>
                  </svg>
                )}
              </button>
              <button
                aria-label="Copy"
                title="Copy"
                className="p-2 rounded-md border border-neutral-700"
                onClick={() => {
                  void navigator.clipboard
                    .writeText(arrayText)
                    .then(() => {
                      setCopiedArray(true);
                      setTimeout(() => setCopiedArray(false), 1500);
                    });
                }}
              >
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="h-4 w-4">
                  <path d="M16 1H4c-1.1 0-2 .9-2 2v12h2V3h12V1zm3 4H8c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h11c1.1 0 2-.9 2-2V7c0-1.1-.9-2-2-2zm0 14H8V7h11v12z"/>
                </svg>
              </button>
              {copiedArray && <div className="text-xs text-green-400">Copied!</div>}
            </div>
          </div>
          <div>
            <div className="text-xs text-neutral-400">Address</div>
            <div className="flex items-center gap-2">
              <div className="flex-1 break-all rounded-md border border-neutral-700 bg-neutral-900 px-3 py-2 text-sm">{address}</div>
              <button
                aria-label="Copy"
                title="Copy"
                className="p-2 rounded-md border border-neutral-700"
                onClick={() => {
                  void navigator.clipboard
                    .writeText(address)
                    .then(() => {
                      setCopiedAddress(true);
                      setTimeout(() => setCopiedAddress(false), 1500);
                    });
                }}
              >
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="h-4 w-4">
                  <path d="M16 1H4c-1.1 0-2 .9-2 2v12h2V3h12V1zm3 4H8c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h11c1.1 0 2-.9 2-2V7c0-1.1-.9-2-2-2zm0 14H8V7h11v12z"/>
                </svg>
              </button>
              {copiedAddress && <div className="text-xs text-green-400">Copied!</div>}
            </div>
          </div>
        </div>
      )}
    </div>
  );
}
